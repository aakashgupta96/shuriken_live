class StartStream
	...
	def self.perform(post_id)
		...
		result = %x[$HOME/bin/ffmpeg -loop 1 -re -y -f image2 -i #{frame_path} -i #{audio_path} -acodec copy -bsf:a aac_adtstoasc -pix_fmt yuv420p -profile:v high -s 1280x720 -vb 400k -maxrate 1500k -minrate 400k -bufsize 6000k -deinterlace -vcodec libx264 -preset veryfast -g 60 -r 30 -t 14400 -strict -2 -f flv "rtmp://rtmp-api.facebook.com:80/rtmp/#{post.key}" 2> #{Rails.root.join("log").join("stream").join(post.id.to_s).to_s} ]
